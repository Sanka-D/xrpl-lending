...
import { AssetIndex, ASSET_SYMBOLS, ASSET_UNIT } from "@/lib/constants";
...
export function SupplyDialog({ asset, open, onClose }: Props) {
  ...
  const handleSupply = async () => {
    ...
    try {
      ...
      await provider.supply(address, asset, native);
      toast.success(`Supplied ${amount} ${ASSET_SYMBOLS[asset]}`);
      onClose();
    } catch (e) {
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="bg-[#0f1011] border border-white/10 text-[#f7f8f8] max-w-sm">
        <DialogHeader>
          <DialogTitle>Supply {ASSET_SYMBOLS[asset]}</DialogTitle>
        </DialogHeader>
        <div className="space-y-4 mt-2">
          ...
          >
            {loading ? "Supplying..." : `Supply ${ASSET_SYMBOLS[asset]}`}
          </Button>
          {!address && <p className="text-xs text-[#ef4444] text-center">Connect wallet first</p>}
        </div>
      </DialogContent>
    </Dialog>
  );
